...
PixelShader = 
{
	...
	[[
		...
		{
            float customState = 0;
            if (v.vTexCoord0.x <= 0.33) {
                customState = min(CurrentState * 3, 1);
            }
            else if (v.vTexCoord0.x <= 0.66) {
                customState = min(max(CurrentState - 0.33, 0) * 3, 1);
            }
            else {
                customState = min(max(CurrentState - 0.66, 0) * 3, 1);
            }
            return customState * tex2D( TextureOne, v.vTexCoord0.xy ) + (1 - customState) * tex2D( TextureTwo, v.vTexCoord0.xy );
        }
	]]
